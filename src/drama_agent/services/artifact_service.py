...
async def create_root(
    ...
    aspect_ratio: str = "", negative_prompt: str | None = None,
    seed: int | None = None, revised_prompt: str | None = None,
) -> dict:
    """写根产物。aspect_ratio / negative_prompt / seed 是**重放这次生成所需**的入参,
    缺一项「重跑」就不是重跑而是另抽一次(seed 由平台回传,见 VideoTaskResult)。"""
    row = VideoArtifact(
...
async def create_child(session: AsyncSession, parent_artifact: dict, new_fields: dict) -> dict:
    row = VideoArtifact(id=str(uuid.uuid4()), **{
        ...
        # 比例/负向提示随父继承:动作没显式改它们就该与父一致,否则重跑出的画面比例都会变
        "aspect_ratio": new_fields.get("aspect_ratio", parent_artifact.get("aspect_ratio") or ""),
        "duration": new_fields["duration"],
        "action": new_fields["action"],
        "negative_prompt": new_fields.get(
            "negative_prompt", parent_artifact.get("negative_prompt")),
        "seed": new_fields.get("seed"),
        "revised_prompt": new_fields.get("revised_prompt"),
